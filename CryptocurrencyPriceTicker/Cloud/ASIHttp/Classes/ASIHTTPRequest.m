...
- (void)startRequest
{
    ...
    //NSLog(@"post body %lld %@ %@",postLength,[[[NSString alloc] initWithData:postBody encoding:NSUTF8StringEncoding] autorelease],requestHeaders);
...
- (void)requestReceivedResponseHeaders:(NSMutableDictionary *)newResponseHeaders
{
    ...
    //NSLog(@"Response %@",newResponseHeaders);
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			NSString *OSName;
			NSString *OSVersion;
			NSString *locale = [[NSLocale preferredLanguages] objectAtIndex:0];//[[NSLocale currentLocale] localeIdentifier];
